...
import { LayoutDashboard, Server, User, Settings, LogOut, Activity, ChevronRight, Gamepad2, Sparkles } from 'lucide-react'
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-transparent text-slate-100">
      <aside className="relative w-72 shrink-0 overflow-hidden border-r border-white/10 bg-slate-950/80 backdrop-blur-2xl">
        ...
        <div className="relative flex h-full flex-col">
            ...
            {navItems.map(({ to, label, icon: Icon }) => (
              <NavLink
                ...
                className={({ isActive }) => cn(
                  ...
                  isActive
                    ? 'bg-gradient-to-r from-emerald-500/20 to-cyan-500/10 text-emerald-100 ring-1 ring-emerald-400/25 shadow-lg shadow-emerald-950/20'
                    : 'text-slate-400 hover:bg-white/5 hover:text-slate-100'
                )}
              >
                {({ isActive }) => (
                  <>
                    <span className={cn('flex h-8 w-8 items-center justify-center rounded-xl border transition-all', isActive ? 'border-emerald-400/30 bg-emerald-400/10' : 'border-white/5 bg-white/[0.03] group-hover:border-white/10')}>
                      <Icon className={cn('w-4 h-4 shrink-0 transition-colors', isActive ? 'text-emerald-300' : 'text-slate-500 group-hover:text-slate-300')} />
                    </span>
                    <span>{label}</span>
          ...
          <div className="px-4 py-5 border-t border-white/10 space-y-4">
            {licence && (
              <div className={cn('flex items-center gap-2 px-3 py-2.5 rounded-2xl border text-xs font-bold backdrop-blur', licenceBadgeClass[licence.status])}>
                <Activity className="w-3.5 h-3.5 shrink-0" />
                <span className="capitalize">{licence.status} licence</span>
                <span className="ml-auto opacity-70">{licence.max_nodes} nodes</span>
              </div>
            )}
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-3">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-slate-700 to-slate-900 overflow-hidden shrink-0 ring-1 ring-white/10">
                {profile?.avatar_url ? <img src={profile.avatar_url} alt={profile.username ?? 'User'} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-300 text-xs font-bold">{(profile?.username ?? 'U')[0].toUpperCase()}</div>}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-slate-100 text-xs font-bold truncate">{profile?.username ?? 'Loading...'}</p>
                <p className="text-slate-500 text-xs">Operator</p>
              </div>
              <button onClick={signOut} className="p-2 rounded-xl text-slate-500 hover:text-red-300 hover:bg-red-500/10 transition-colors shrink-0" title="Sign out">
      ...
      <main className="relative flex-1 overflow-y-auto">
        <div className="pointer-events-none fixed right-8 top-8 h-72 w-72 rounded-full bg-emerald-500/10 blur-3xl" />
        <Outlet />
      </main>
    </div>
  )
