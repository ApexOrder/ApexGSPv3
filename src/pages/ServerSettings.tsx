...
export default function ServerSettings() {
  ...
  async function saveSettings() {
    ...
    setSaving(true)
    setMessage('Saving config through Direct API...')

    try {
      const result = await callNodeApi<ConfigResult>(session, 'config', {
        server_id: server.id,
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Instant Direct API save to serverconfig.xml for {server.name}</p>
        </div>
        <button onClick={saveSettings} disabled={saving} className="flex items-center gap-2 bg-brand-600 hover:bg-brand-500 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 mb-6 flex items-center justify-between gap-4">
        ...
        {lastSaved && <p className="text-xs text-emerald-400 font-mono">saved {lastSaved}</p>}
      </div>
      ...
      <div className="mt-4 rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
        <p className="text-xs text-amber-300 font-medium">Some settings may require a server restart before 7DTD applies them.</p>
      </div>
    </div>
  )
}
