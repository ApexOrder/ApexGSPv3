...
import { formatTimeZoneLabel, getPanelTimeZone } from '@/lib/timezone'
...
export default function ServerScheduler() {
  ...
  async function saveSchedule() {
    ...
    setBusy(true)
    try {
      const result = await direct('schedule_save', { backupMode, frequency, time, dayOfWeek, retention, enabled, timeZone: panelTimeZone })
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4"><div className="md:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-4"><p className="text-xs text-slate-400">Direct API: <span className="text-slate-200 font-mono">{message || 'Ready'}</span></p></div><div className="bg-slate-900 border border-slate-800 rounded-xl p-4"><p className="text-xs text-slate-500 mb-1">Panel scheduler timezone</p><p className="text-sm text-slate-200 font-mono">{formatTimeZoneLabel(panelTimeZone)}</p><p className="text-xs text-slate-500 mt-1">Daemon reports: {daemonTime ? `${daemonTime.timeZone} • ${daemonTime.offset}` : 'Loading...'}</p></div></div>
